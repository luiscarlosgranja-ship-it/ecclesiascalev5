...
import { Plus, Printer, Zap, Trash2, CheckCircle, Repeat, Calendar, Lock, Unlock } from 'lucide-react';
...
export default function ScalesPage({ user }: Props) {
  ...
  // ─── Travar/destravar vaga ───────────────────────────────────────────────────
  async function toggleLock(s: Scale) {
    await api.put(`/scales/${s.id}/lock`, { locked: !s.locked });
    refetchScales();
  }
                    ...
                    <tr key={s.id} className="border-b border-stone-800 hover:bg-stone-800/30 transition-colors">
                      ...
                      <td className="p-3">
                        <div className="flex justify-end gap-1">
                          {s.locked && (
                            <span title="Vaga travada" className="text-amber-400 p-1">
                              <Lock size={14} />
                            </span>
                          )}
                          {s.status === 'Pendente' && (
                          ...
                          {(isAdmin(user.role) || isLeader(user.role)) && (
                            <>
                              <button onClick={() => toggleLock(s)} title={s.locked ? 'Destravar vaga' : 'Travar vaga'}
                                className={`p-1 transition-colors ${s.locked ? 'text-amber-400 hover:text-amber-300' : 'text-stone-500 hover:text-amber-400'}`}>
