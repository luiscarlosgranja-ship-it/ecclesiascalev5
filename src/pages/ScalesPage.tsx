...
export default function ScalesPage({ user }: Props) {
  ...
  async function fillCult() {
    ...
    try {
      ...
      setFillMsg(res.message || `${res.created} escalas criadas`);
      await refetchScales();
      await fetchDeptBlocks();
    } catch (e) {
  ...
  async function removeScale(id: number) {
    ...
    const deleteCult  = remaining.length === 0 ? selectedCult : null;
    if (deleteCult) { suppressRT.current = true; setSelectedCult(null); setDeptBlocks([]); }
    try {
      await api.delete(`/scales/${id}`);
      if (deleteCult) { await api.delete(`/cults/${deleteCult}`); await refetchCults(); }
      else { await refetchScales(); await fetchDeptBlocks(); }
    } catch (e) {
      if (deleteCult) setSelectedCult(deleteCult);
      alert(e instanceof Error ? e.message : 'Erro ao remover');
    } finally {
      if (deleteCult) setTimeout(() => { suppressRT.current = false; }, 1500);
    }
  }

  // ── remover escala inteira ─────────────────────────────────────────────────
  async function deleteEntireScale() {
    if (!selectedCult) return;
    setDeleteLoading(true);
    const cultId = selectedCult;
    suppressRT.current = true;
    try {
      const fresh = await api.get<{ id: number }[]>(`/scales?cult_id=${cultId}`);
      if (fresh && fresh.length > 0) await Promise.all(fresh.map(s => api.delete(`/scales/${s.id}`)));
      await api.delete(`/cults/${cultId}`);
      ...
      setTimeout(() => { suppressRT.current = false; }, 1500);
    }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Preencher Automático */}
      <Modal open={fillModal} onClose={() => { setFillModal(false); setFillMsg(''); }}
        title="Preencher Voluntários Automaticamente" size="sm">
        <div className="space-y-4">
          {selectedCultData && (
            <div className="bg-stone-800 border border-stone-600 rounded-lg p-3 text-xs space-y-1">
              <p><span className="text-stone-200 font-semibold">Culto:</span>{' '}
                <span className="text-stone-300">{selectedCultData.name || selectedCultData.type_name}</span></p>
              <p><span className="text-stone-200 font-semibold">Data:</span>{' '}
                <span className="text-stone-300">{selectedCultData.date}</span></p>
            </div>
          )}
          <div className="bg-stone-800 border border-stone-600 rounded-lg p-3 space-y-2">
            <p className="text-stone-200 text-xs font-semibold">Regras aplicadas:</p>
            <ul className="text-stone-300 text-xs space-y-1 list-disc list-inside leading-relaxed">
              <li>Não escala o mesmo voluntário duas vezes</li>
              <li>Máximo 3 escalas por voluntário no mês</li>
              <li>Setores já preenchidos são mantidos</li>
            </ul>
          </div>
          {fillMsg && (
      ...
      {/* Remover Escala Inteira */}
      <Modal open={deleteModal} onClose={() => setDeleteModal(false)} title="Remover Escala" size="sm">
        <div className="space-y-4">
          <div className="flex items-start gap-3 bg-red-900/20 border border-red-700/40 rounded-lg p-4">
            ...
            <div>
              ...
              <p className="text-red-400/80 text-xs mt-1">Todos os voluntários serão removidos e o culto excluído.</p>
            </div>
          </div>
          {selectedCultData && (
            <div className="bg-stone-800/50 rounded-lg p-3 text-xs text-stone-400 space-y-1">
              ...
              <p><span className="text-stone-300">Voluntários:</span> {scales?.length ?? 0}</p>
            </div>
          )}
          <div className="flex gap-3">
