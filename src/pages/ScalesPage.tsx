            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
              {deptBlocks
                .map((block, idx) => {
                  ...
                  const filledCount = block.scales.filter(s => s.status !== 'Vazio').length;
                  const totalSlots = block.scales.length;
                  return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs opacity-70 ml-1">
                            {filledCount}/{totalSlots} • {confirmedCount} confirmado(s)
                          </span>
                        </div>
                      ...
                      {!isCollapsed && (
                        <div className="divide-y divide-stone-800/60">
                          {block.scales.map((s, i) => (
                            <div key={s.id ?? `empty-${s.sector_id}`} className={`flex items-center justify-between px-4 py-2.5 hover:bg-stone-800/20 transition-colors ${s.status === 'Vazio' ? 'opacity-60' : ''}`}>
                              <div className="flex items-center gap-3 min-w-0">
                                <span className="text-stone-600 text-xs w-4 flex-shrink-0">{i + 1}</span>
                                <div className="min-w-0">
                                  {s.status === 'Vazio' ? (
                                    <p className="text-stone-600 text-sm italic">— Vazio —</p>
                                  ) : (
                              ...
                              <div className="flex items-center gap-1 flex-shrink-0 ml-2">
                                ...
                                {s.status === 'Vazio' && canManage && (
                                  <span className="text-xs text-stone-600 italic">sem voluntário</span>
                                )}
                                {isAdmin(user.role) && s.id && (
                                  <>
                                    {s.status === 'Pendente' && (
        ...
        <div className="space-y-4">
          {selectedCultData && (
            <div className="bg-stone-800 border border-stone-600 rounded-lg p-3 text-xs space-y-1">
              <p><span className="text-stone-200 font-semibold">Culto:</span> <span className="text-stone-300">{selectedCultData.name || (selectedCultData as any).type_name}</span></p>
          ...
          <div className="bg-stone-800 border border-stone-600 rounded-lg p-3 space-y-2">
            ...
            <ul className="text-stone-300 text-xs space-y-1 list-disc list-inside leading-relaxed">
              <li>Preenche apenas setores ainda sem voluntário</li>
              <li>Máximo 3 escalas por voluntário no mês</li>
              <li>Respeita o departamento de cada setor</li>
            </ul>
          </div>
          {fillMsg && (
