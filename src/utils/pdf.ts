  ...
  for (let i = 0; i < deptBlocks.length; i++) {
    ...
    sectors.forEach((sector) => {
      ...
      sector.scales.forEach((scale, idx) => {
        ...
        // Name column (left ~55% of block)
        const nameColW = (blockW - blockPadding * 2) * 0.55;
        const sectorColW = (blockW - blockPadding * 2) * 0.45 - 2;
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(5);
        doc.setTextColor(50, 45, 40);
        doc.text(scale.member_name || '—', bx + blockPadding, contentY + 2.5, { maxWidth: nameColW });

        // Sector column (right of name, left-aligned)
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(4.5);
        doc.setTextColor(100, 90, 80);
        doc.text(scale.sector_name || '—', bx + blockPadding + nameColW + 1, contentY + 2.5, { maxWidth: sectorColW });
